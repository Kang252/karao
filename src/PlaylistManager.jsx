...
export function PlaylistManager({library,onAdd,onAddAll,pendingSong,onCancelSong}) {
  ...
  return <section className="playlist-manager">
    ...
    <div className="playlist-tabs">{library.playlists.map(p=><button type="button" key={p.id} className={current?.id===p.id?'active':''} onClick={()=>{setSelected(p.id);setEditing(null);setDeleting(null)}}>{p.name} <span>{p.songs.length}</span></button>)}</div>
    ...
    {current && <div className="playlist-detail">
      ...
      <div className="saved-favorites">{current.songs.map(s=><article key={s.id}><div><strong>{s.title}</strong><small>{s.artist}</small></div><button type="button" onClick={()=>onAdd(s)}><Plus/> Hát</button><button type="button" disabled={library.busy} onClick={()=>library.removeSong(current.id,s.id)} aria-label={'Bỏ '+s.title+' khỏi danh sách'}><Trash2/></button></article>)}</div>
